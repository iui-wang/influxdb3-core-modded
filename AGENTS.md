...
- `influxdb3/` — the `influxdb3` binary (default workspace member) and its library `influxdb3_lib`. Entry point `influxdb3/src/main.rs` → `startup()`; CLI subcommands in `influxdb3/src/commands/`.
- `influxdb3_*/` — v3 server crates: catalog, WAL, write path, query executor, processing engine (embedded Python VM), HTTP server, CLI config blocks, telemetry, test helpers.
...
- System dependencies: `protoc` and `python3` with dev headers — the processing engine embeds Python via PyO3 and dynamically links `libpython` (see `README_processing_engine.md`).
